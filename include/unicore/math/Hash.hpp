...
namespace unicore
{
	...
	public:
		...
		static constexpr UInt32 compute_single(const T data, UInt32 crc)
		{
			...
			{
				crc = table[((data >> 8) & 0xFF) ^ (crc & 0xFF)] ^ (crc >> 8);
				return table[(data & 0xFF) ^ (crc & 0xFF)] ^ (crc >> 8);
			}

			if constexpr (Bytes == 4)
			{
				crc = table[((data >> 24) & 0xFF) ^ (crc & 0xFF)] ^ (crc >> 8);
				crc = table[((data >> 16) & 0xFF) ^ (crc & 0xFF)] ^ (crc >> 8);
				crc = table[((data >> 8) & 0xFF) ^ (crc & 0xFF)] ^ (crc >> 8);
				return table[(data & 0xFF) ^ (crc & 0xFF)] ^ (crc >> 8);
			}
