...
namespace unicore
{
	...
	Optional<RaycastHit> Raycast::cast_ray(const Ray2& ray, const IRaycastWorld& world)
	{
		const Vector2f unit_step = {
			Math::sqrt(1 + Math::pow(ray.direction.y / ray.direction.x)),
			Math::sqrt(1 + Math::pow(ray.direction.x / ray.direction.y))
		...
		while (true)
		{
			if (ray_length.x < ray_length.y)
			{
				...
				if (!world.is_solid(map_check))
				{
					RaycastHit hit;
					hit.index = map_check;
					hit.distance = ray_length.x;
					return hit;
				}
